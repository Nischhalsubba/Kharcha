...
function BreakdownTabs({value,onChange}){
  return <View style={s.breakdownTabs}>{['expense','budget','income'].map(key=><HapticPressable key={key} onPress={()=>onChange(key)} style={[s.breakdownTab,value===key&&s.breakdownTabActive]} accessibilityRole="tab" accessibilityState={{selected:value===key}}><Text style={[s.breakdownTabText,value===key&&s.breakdownTabTextActive]}>{key==='expense'?'Expenses':key==='income'?'Income':'Budget'}</Text></HapticPressable>)}</View>;
}
...
function BudgetRows({categoryBudgets,m,settings,onEdit,onRemove}){
  ...
  return <View>{categoryBudgets.map((item,index)=><Pressable key={item.category} onPress={()=>onEdit(item.category)} style={[s.breakdownRow,index>0&&s.breakdownRowBorder]}>
    ...
    {onRemove?<HapticPressable haptic={null} onPress={()=>onRemove(item.category)} hitSlop={10} accessibilityLabel="Remove category budget"><NativeIcon name="close" size={18} color={COLORS.danger}/></HapticPressable>:null}
  </Pressable>)}</View>;
}
...
export default function BudgetScreen({
  ...
  if(view==='management')return <ScrollView contentInsetAdjustmentBehavior="automatic" showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
    ...
    {(settings.customCategories.expense.length||settings.customCategories.income.length)?<><Section title={lang==='ne'?'आफ्नै श्रेणीहरू':'Custom categories'}/><View style={s.list}>{['expense','income'].flatMap(type=>(settings.customCategories[type]||[]).map(item=><View key={item.id} style={s.manageRow}><View style={s.customCategoryTitle}><View style={s.icon}><NativeIcon name="tag" size={18} color="#0074FC"/></View><Text style={s.rowTitle}>{item.name}</Text></View><View style={s.manageRight}><Text style={s.meta}>{type}</Text><HapticPressable haptic={null} onPress={()=>removeCategory(type,item.id,item.name)}><Text style={s.delete}>{t(lang,'remove','Remove')}</Text></HapticPressable></View></View>))}</View></>:null}
  </ScrollView>;
