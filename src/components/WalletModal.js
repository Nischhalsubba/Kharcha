...
export default function WalletModal({ visible, onClose, onSave, language = 'en' }) {
  ...
  return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
    <Pressable style={s.backdrop} onPress={onClose}/><View style={s.sheet}><Text style={s.title}>{t(language,'addWallet','Add wallet')}</Text><Text style={s.muted}>{language==='ne'?'नगद, बैंक र डिजिटल वालेट छुट्टाछुट्टै ट्र्याक गर्नुहोस्।':'Track cash, bank and digital wallets separately.'}</Text>
      <Text style={s.label}>{language==='ne'?'प्रकार':'Type'}</Text><ScrollView horizontal showsHorizontalScrollIndicator={false}>{WALLET_PRESETS.map(([id,label])=><HapticPressable key={id} onPress={()=>choose(id)} style={[s.chip,preset===id&&s.selected]}><NativeIcon name={walletIconName({type:id,id})} size={17} color={preset===id?COLORS.accent:COLORS.mutedStrong}/><Text style={s.chipText}>{label}</Text></HapticPressable>)}</ScrollView>
      ...
      <HapticPressable haptic="impact" style={s.primary} onPress={submit}><Text style={s.primaryText}>{t(language,'addWallet','Add wallet')}</Text></HapticPressable>
    </View>
  </Modal>;
}
const s=StyleSheet.create({backdrop:{...StyleSheet.absoluteFillObject,backgroundColor:'rgba(38,39,48,.42)'},sheet:{position:'absolute',left:0,right:0,bottom:0,backgroundColor:COLORS.surface,borderTopLeftRadius:20,borderTopRightRadius:20,borderWidth:1,borderColor:COLORS.border,padding:20,paddingBottom:28},title:{color:COLORS.text,fontSize:22,fontWeight:'600'},muted:{color:COLORS.muted,fontSize:13,marginTop:4},label:{color:COLORS.text,fontSize:12,fontWeight:'600',marginTop:16,marginBottom:7},chip:{height:44,paddingHorizontal:12,borderRadius:10,borderWidth:1,borderColor:COLORS.border,backgroundColor:COLORS.surface,flexDirection:'row',gap:6,alignItems:'center',marginRight:8},selected:{borderColor:COLORS.accent,backgroundColor:COLORS.accentSoft},chipText:{color:COLORS.text,fontSize:12,fontWeight:'600'},input:{height:48,borderRadius:10,borderWidth:1,borderColor:COLORS.border,backgroundColor:COLORS.surface2,color:COLORS.text,paddingHorizontal:12,fontVariant:['tabular-nums']},primary:{height:48,borderRadius:10,marginTop:20,backgroundColor:COLORS.accent,alignItems:'center',justifyContent:'center'},primaryText:{color:COLORS.onAccent,fontSize:15,fontWeight:'600'}});
