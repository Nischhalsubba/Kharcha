...
import { COLORS, EXPENSE_CATEGORIES, INCOME_CATEGORIES } from '../constants';
...
export default function AddTransactionModal({ visible, onClose, onSave }) {
  ...
  useEffect(() => {
    if (!visible) return;
    setType('expense'); setAmount(''); setCategory('Food'); setNote('');
    setDate(todayIso()); setSaving(false);
  }, [visible]);

  const categories = type === 'expense' ? EXPENSE_CATEGORIES : INCOME_CATEGORIES;
  const switchType = (next) => { setType(next); setCategory(next === 'expense' ? 'Food' : 'Salary'); };
  ...
  async function submit() {
    ...
    await onSave({
      ...
      type, amount: normalized, category, note: note.trim(), date,
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <KeyboardAvoidingView style={s.root} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <View style={s.sheet}>
          <View style={s.handle} />
          <View style={s.header}>
            <View><Text style={s.title}>Add transaction</Text><Text style={s.muted}>Takes only a few seconds.</Text></View>
            <Pressable onPress={onClose} hitSlop={12} accessibilityRole="button" accessibilityLabel="Close add transaction"><Text style={s.close}>×</Text></Pressable>
          </View>
          ...
          <View style={s.amountWrap}><Text style={s.prefix}>Rs</Text><TextInput value={amount} onChangeText={setAmount} keyboardType="decimal-pad" placeholder="0" placeholderTextColor={COLORS.muted} style={s.amount} autoFocus /></View>
          ...
          <Pressable style={[s.primary, saving && s.disabled]} onPress={submit} disabled={saving} accessibilityRole="button"><Text style={s.primaryText}>{saving ? 'Saving…' : `Save ${type}`}</Text></Pressable>
        </View>
...
const s = StyleSheet.create({
  root:{flex:1,justifyContent:'flex-end'}, backdrop:{...StyleSheet.absoluteFillObject,backgroundColor:'rgba(0,0,0,.58)'},
  sheet:{backgroundColor:'#10161E',borderTopLeftRadius:28,borderTopRightRadius:28,borderWidth:1,borderColor:COLORS.border,paddingHorizontal:20,paddingTop:10,paddingBottom:Platform.OS==='ios'?30:22},
  handle:{width:42,height:4,borderRadius:99,backgroundColor:COLORS.border,alignSelf:'center',marginBottom:14}, header:{flexDirection:'row',justifyContent:'space-between'},
  title:{color:COLORS.text,fontSize:22,fontWeight:'900'}, muted:{color:COLORS.muted,fontSize:13,marginTop:4}, close:{color:COLORS.muted,fontSize:30,lineHeight:30},
  ...
  twoCol:{flexDirection:'row',gap:10},flex:{flex:1},input:{height:48,borderRadius:14,borderWidth:1,borderColor:COLORS.border,backgroundColor:COLORS.surface2,color:COLORS.text,paddingHorizontal:12,fontSize:14},
  primary:{height:52,borderRadius:15,marginTop:18,backgroundColor:COLORS.accent,alignItems:'center',justifyContent:'center'},primaryText:{color:'#07130F',fontSize:15,fontWeight:'900'},disabled:{opacity:.6},
});
