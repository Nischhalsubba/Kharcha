...
export default function NepalSettingsModal({ visible, onClose, settings, onSave, onDataSafety }) {
  ...
  return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}><Pressable style={s.backdrop} onPress={onClose}/><View style={s.sheet}>
    ...
    <Pressable style={s.safety} onPress={onDataSafety}><Text style={s.safetyText}>🛟  {t(lang,'dataSafety','Data & safety')}</Text></Pressable><Pressable style={s.primary} onPress={()=>onSave({...settings,language,dateSystem,amountFormat})}><Text style={s.primaryText}>{t(lang,'save','Save')}</Text></Pressable>
  </View></Modal>;
}
const s=StyleSheet.create({backdrop:{...StyleSheet.absoluteFillObject,backgroundColor:'rgba(38,39,48,.42)'},sheet:{position:'absolute',left:0,right:0,bottom:0,backgroundColor:COLORS.surface,borderTopLeftRadius:20,borderTopRightRadius:20,borderWidth:1,borderColor:COLORS.border,padding:20,paddingBottom:28},title:{color:COLORS.text,fontSize:22,fontWeight:'600'},muted:{color:COLORS.muted,fontSize:13,lineHeight:19,marginTop:4},label:{color:COLORS.text,fontSize:12,fontWeight:'600',marginTop:18,marginBottom:8},row:{flexDirection:'row',gap:8},choice:{flex:1,minHeight:44,borderRadius:10,borderWidth:1,borderColor:COLORS.border,backgroundColor:COLORS.surface,alignItems:'center',justifyContent:'center',paddingHorizontal:8},choiceActive:{borderColor:COLORS.accent,backgroundColor:COLORS.accentSoft},choiceText:{color:COLORS.muted,fontWeight:'600',fontSize:12},choiceTextActive:{color:COLORS.text},safety:{height:48,borderRadius:10,marginTop:18,borderWidth:1,borderColor:COLORS.border,backgroundColor:COLORS.surface2,alignItems:'center',justifyContent:'center'},safetyText:{color:COLORS.text,fontSize:13,fontWeight:'600'},primary:{height:48,borderRadius:10,marginTop:10,backgroundColor:COLORS.accent,alignItems:'center',justifyContent:'center'},primaryText:{color:COLORS.onAccent,fontSize:15,fontWeight:'600'}});
