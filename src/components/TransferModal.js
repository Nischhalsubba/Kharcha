...
export default function TransferModal({visible,onClose,onSave,wallets=[],settings={}}) {
  ...
  return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
    ...
    <View style={s.sheet}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>{available.map(wallet=><Pressable key={wallet.id} onPress={()=>{setFromWalletId(wallet.id);if(toWalletId===wallet.id)setToWalletId(available.find(item=>item.id!==wallet.id)?.id||'');}} style={[s.wallet,fromWalletId===wallet.id&&s.walletActive]}><Text style={s.walletText}>{wallet.icon||'👛'} {wallet.name}</Text></Pressable>)}</ScrollView>
      <Text style={s.label}>{t(lang,'toWallet','To wallet')}</Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>{available.filter(wallet=>wallet.id!==fromWalletId).map(wallet=><Pressable key={wallet.id} onPress={()=>setToWalletId(wallet.id)} style={[s.wallet,toWalletId===wallet.id&&s.walletActive]}><Text style={s.walletText}>{wallet.icon||'👛'} {wallet.name}</Text></Pressable>)}</ScrollView>
