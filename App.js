...
export default function App(){
  ...
  if(!ready||!securityReady)return <SafeAreaProvider><SafeAreaView style={[s.safe,s.center]} edges={['top','left','right','bottom']}><StatusBar barStyle="dark-content"/><Image source={require('./assets/images/kharcha-logo-horizontal.png')} style={s.loadingLogo} resizeMode="contain"/><Text style={s.meta}>Loading your money view…</Text></SafeAreaView></SafeAreaProvider>;
  ...
  return <SafeAreaProvider><SafeAreaView style={s.safe} edges={['top','left','right','bottom']}><StatusBar barStyle="dark-content" backgroundColor={COLORS.bg}/><View style={s.app}>
    <View style={s.top}><View style={s.brandLockup}><Image source={require('./assets/images/kharcha-logo-horizontal.png')} style={s.brandLogo} resizeMode="contain"/><Text style={s.eyebrow}>{t(lang,'personalMoney','PERSONAL MONEY').toUpperCase()}</Text></View><Pressable style={s.badge} onPress={()=>setSettingsOpen(true)}><Text style={s.dot}>●</Text><Text style={s.badgeText}>{lang==='ne'?'नेपाली · ':'EN · '}{settings.dateSystem}</Text></Pressable></View>
