...
export default function App(){
  ...
  if(!ready||!securityReady)return <SafeAreaProvider><SafeAreaView style={[s.safe,s.center]} edges={['top','left','right','bottom']}><StatusBar barStyle="dark-content"/><View style={s.loadingBrand}><Image source={require('./assets/images/splash-icon.png')} style={s.brandMark} resizeMode="contain"/><Text style={s.brand}>Kharcha</Text></View><Text style={s.meta}>Loading your money view…</Text></SafeAreaView></SafeAreaProvider>;
  if(locked)return <SafeAreaProvider><SafeAreaView style={s.safe} edges={['top','left','right','bottom']}><LockScreen onUnlockPin={unlockWithPin} onUnlockBiometric={unlockWithBiometric} biometricEnabled={securityConfig.biometricEnabled} biometricAvailable={biometricAvailable} cooldownUntil={cooldownUntil} failedAttempts={failedAttempts}/></SafeAreaView></SafeAreaProvider>;
  const lang=settings.language;
  const m=(value)=>money(value,settings);
  return <SafeAreaProvider><SafeAreaView style={s.safe} edges={['top','left','right','bottom']}><StatusBar barStyle="dark-content" backgroundColor={COLORS.bg}/><View style={s.app}>
    <View style={s.top}><View style={s.brandRow}><Image source={require('./assets/images/splash-icon.png')} style={s.brandMark} resizeMode="contain"/><Text style={s.brand}>Kharcha</Text></View><HapticPressable style={s.headerAction} onPress={()=>setSettingsOpen(true)} accessibilityRole="button" accessibilityLabel={t(lang,'settings','Settings')}><NativeIcon name="settings" size={22} color={COLORS.text}/></HapticPressable></View>
    ...
    <NativeTabBar activeTab={tab} onTabChange={setTab} onAdd={()=>openAdd()} items={[{key:'overview',icon:'home',label:t(lang,'overview','Overview')},{key:'activity',icon:'activity',label:t(lang,'activity','Activity')},{key:'budget',icon:'budget',label:t(lang,'budget','Budget')},{key:'insights',icon:'reports',label:t(lang,'reports','Reports')}]}/>
