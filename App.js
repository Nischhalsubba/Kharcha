...
import { Alert, AppState, Image, Pressable, StatusBar, Text, View } from 'react-native';
...
export default function App(){
  ...
  return <SafeAreaProvider><SafeAreaView style={s.safe} edges={['top','left','right','bottom']}><StatusBar barStyle="dark-content" backgroundColor={COLORS.bg}/><View style={s.app}>
    <View style={s.top}><View style={s.brandLockup}><View style={s.brandRow}><Image source={require('./assets/images/splash-icon.png')} style={s.brandMark} resizeMode="contain"/><Text style={s.brand}>Kharcha</Text></View><Text style={s.eyebrow}>{t(lang,'personalMoney','PERSONAL MONEY').toUpperCase()}</Text></View><Pressable style={s.badge} onPress={()=>setSettingsOpen(true)}><Text style={s.dot}>●</Text><Text style={s.badgeText}>{lang==='ne'?'नेपाली · ':'EN · '}{settings.dateSystem}</Text></Pressable></View>
